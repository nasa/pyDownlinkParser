...
class RICEDecompressor(Converter):
    ...
    def convert(self, sciotype, sciofrag, data_array):
        ...
        for data in data_array:
            deconpressed_field = self.convert_row(data)
            decompressed_data.append(deconpressed_field)

        return decompressed_data

    def convert_row(self, in_bit_stream: list) -> bitstring.ConstBitStream:
        """RICE decompression."""
        output = []
        n_bits = 12
    ...
    def set_pre_post(self, df: pandas.DataFrame):
        self.ls_post = df["NBLOCKS_LS_POST"]
        self.ls_pre = df["NBLOCKS_LS_POST"]
        self.hs_post = df["NBLOCKS_HS_POST"]
        self.hs_pre = df["NBLOCKS_HS_PRE"]
