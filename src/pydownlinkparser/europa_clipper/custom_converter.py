...
class RICEDecompressor(converters):
    ...
    def rice_decode_linear(self, in_bit_stream: bitstring.ConstBitStream, n_bits=12, sf_size=SF_SIZE_BITS,
        ...
        try:
            ...
            for _ in range(linear_number):
                sample = in_bit_stream.read(12).uint
                output.append(sample)

            while len(output) < sf_size:
                residual = self.rice_decode(in_bit_stream, 12, rice_k)
    ...
    def convert(self, in_bit_stream: bitstring.ConstBitStream) -> bitstring.ConstBitStream:

        output = []

        rice_k_bits = math.ceil(math.log2(12))
        ...
        try:
            while len(output) < sample_count:
                ...
                if predictor == 0b00:  # Constant
                    frame = self.constant(in_bit_stream, n_bits=12, sf_size=SF_SIZE_BITS)
                elif predictor == 0b01:  # Verbatim
                    frame = self.verbatim(in_bit_stream, n_bits=12, sf_size=SF_SIZE_BITS)
                elif predictor == 0b10:  # Linear Predictor #1
                    frame = self.rice_decode_linear(in_bit_stream, n_bits=12, sf_size=SF_SIZE_BITS,
